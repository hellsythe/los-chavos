...
class Payment extends BaseModel
{
    ...
    {
        $payment_available = floatval($this->amount);

        $order = Order::findModel($this->order_id);
        $order->missing_embroidery = floatval($order->missing_embroidery);
        $order->missing_print = floatval($order->missing_print);
        ...
        if ($payment_available > 0 && $order->missing_print > 0) {
